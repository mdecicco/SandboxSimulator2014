...
namespace SandboxSimulator
{
    ...
    void Mesh::SetVertex(i32 Idx,const Vec3& v)
    {
        m_Vertices[Idx] = v;
        m_VerticesUpdated = true;
        m_HasChanged = true;
    }
    Vec3 Mesh::GetVertex(i32 Idx) const
    {
    ...
    void Mesh::SetTexCoord(i32 Idx,const Vec2& t)
    {
        m_TexCoords[Idx] = t;
        m_TexCoordsUpdated = true;
    }
    Vec2 Mesh::GetTexCoord(i32 Idx) const
    {
    ...
    void Mesh::SetNormal(i32 Idx,const Vec3& n)
    {
        m_Normals[Idx] = n;
        m_NormalsUpdated = true;
    }
    Vec3 Mesh::GetNormal(i32 Idx) const
    {
        return m_Normals[Idx];
    }

    void Mesh::AddTangent(const Vec3& t)
    {
        m_Tangents.push_back(t);
        m_TangentsUpdated = true;
    }
    void Mesh::SetTangent(i32 Idx,const Vec3& t)
    {
        m_Tangents.push_back(t);
        if(m_TangBuff)
        {
    ...
    void Mesh::SyncBuffers()
    {
        ...
        if(m_VertBuff + m_TexCBuff + m_NormBuff + m_TangBuff == 0 && m_VAO)
        {
            glDeleteVertexArrays(1,&m_VAO);
            m_VAO = 0;
            if(m_Vertices.size() + m_TexCoords.size() + m_Normals.size() + m_Tangents.size() == 0) return;
        }
    ...
    void Mesh::SetTexParam(i32 TexNum,GLenum Param,GLint Value)
    {
        ...
        {
            printf("Error setting texture parameter. Texture[%d] does not exist.\n",TexNum);
            return;
        }
        m_TexParams.push_back(iv3(TexNum,(i32)Param,Value));
    }
    void Mesh::GetMinMaxVertices(Vec3& Min,Vec3& Max)
    {
